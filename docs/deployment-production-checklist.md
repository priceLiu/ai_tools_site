...
2. **生产环境应配置的变量（名称示例）**  
   ...
   - **`SITE_URL`（首选）或 `NEXT_PUBLIC_SITE_URL`**：生产站点绝对根 URL（无尾斜杠），用于 canonical、sitemap、OG（见 [`lib/site-url.ts`](../lib/site-url.ts)）。**腾讯云 / 自建部署没有 `VERCEL_URL`**，`getSiteUrl()` 不会自动推断域名；不配此项时会错误回落到仓库里的 Vercel 占位域名，SEO 与分享卡片链接全部失真。**只需配 `SITE_URL` 即可**（当前代码里 `getSiteUrl` 仅在服务端使用，不必强求 `NEXT_PUBLIC_*`）。**请填写 `https://` 域名**；若误写 `http://`，生产构建会在 [`getSiteUrl()`](../lib/site-url.ts) 中自动改为 `https://`。  
