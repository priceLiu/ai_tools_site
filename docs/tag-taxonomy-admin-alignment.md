# 标签 / 场景 / 角色 / 菜单分类 — 需求整理与实施计划

> **文档用途**：固化你与助手上一轮讨论的结论，标明「目标信息架构」「现状差距」「分阶段勾选清单」，后续迭代以此为准逐项勾选。
>
> **历史上下文**：场景与导入管线详见 [`docs/label.md`](./label.md)。

> **里程碑状态（2026-05-07）**：本节第五、六部分的 **Phase 0–5 已全部勾选完成**。下文 **§1.2** 保留的是「立项前核对」结论，不是当前缺口；若只看「有没有做完」，以 **§五 / §六** 为准。

> **与现状代码对齐**：`/category` 工具列表已以 **`tool_categories`** junction 为主（辅以 `tools.category_id` 作展示主分类等）；下文个别段落仍写「仅靠 category_id」处请以 **`neonListToolsForCategoryIds`** 与迁移 **`20260507160000_tool_categories.sql`** 为准，并逐步收敛正文。**Cursor 强制流程**：开发前先读 [`.cursor/rules/taxonomy-tags-tools-categories.mdc`](../.cursor/rules/taxonomy-tags-tools-categories.mdc)。**工具 / 标签 / 场景 / 角色的联结与计数口径**（收录工具数、Tab 双数字、挂载 vs 关联）：见 **[`docs/tool-tag-taxonomy-counting.md`](./tool-tag-taxonomy-counting.md)**。

---

## 一、原始需求与先前结论（整理）

### 1.1 你对业务的设定（目标语义）

1. **场景分类、角色分类、左侧菜单分类**都可以「聚合一批标签」，也可以与标签在管理上分开维护（松散耦合）。
2. **工具**挂多个标签；可随时清空或重写标签集合。
3. **标签 ↔ 场景、标签 ↔ 角色**都是松散关系（非强制唯一归属）。
4. **场景 / 角色 / 左侧菜单**在语义上是三类不同的「导航聚合维度」，其中左侧菜单**不等价于**「标签的简单别名」，需在模型与文档里**单独说清楚**，不要把三套维度糊成一套。
5. **理想的浏览路径**包括：
   - 场景 → 标签 → 工具  
   - 标签 → 工具  
   - 角色 → 标签 → 工具  
   - 工具 → 标签 → 场景 / 角色（通过详情页的跳转）

### 1.2 立项前核对结论（归档，非「当前待办」）

> 下表是 **写本计划当时** 的结论快照，用于说明为什么要做 Phase 1–5。**不代表现在仍未完成**——菜单↔标签联结、禁用逻辑等已在代码与迁移中落地（见 `docs/done.md` 2026-05-07）。

| 设定项 | 当时结论 | 说明 |
|--------|----------|------|
| 场景 ↔ 标签 | 基本一致 | `tags.tag_category_id` → `tag_categories` |
| 角色 ↔ 标签 | 基本一致 | `role_category_tags` |
| 工具 ↔ 标签 | 一致 | `tool_tags`，最多 20 |
| 左侧菜单 ↔ 标签 | **当时缺联结与后台** | 已通过 `category_tags` + `/admin/menu-categories` 补齐；`/category` 列表仍按 `category_id`（未改并集策略）。 |
| 详情页「工具 → 标签 → 场景/角色」 | **刻意延后** | 仍为 **§四 FR-TAXO-01**，未在本里程碑验收。 |

### 1.3 本轮硬性约束（你补充的规则）

- **前四点**：产品与模型语义要和上述设定对齐；实施上分阶段勾选。
- **第 5 点（详情路径）**：写入 **「待完成需求」**，本阶段不做验收。
- **第 4 点（左侧菜单）**：要把「菜单分类」和「标签库」的关系 **解耦写清楚**，但 **默认前台列表行为与现在一致**（仍主要靠 `tools.category_id`）；新增的标签联结先做 **管理能力**，是否在 `/category` 列表中与「按 category_id 收录」做 **并集** 或其它策略，在计划中单独一步评审后再改查询（避免无声改变线上口径）。
- **左侧菜单后台**：新增 **与「场景分类管理」同构** 的「菜单分类管理」（禁用、往分类里加标签、移除标签）。
- **预留**：用户 **收藏**「场景分类」「角色分类」（以及后续可扩展菜单分类）；本文件 **仅预留章节**，不纳入当前里程碑验收。

---

## 二、目标信息架构（对齐后的四条轴线）

```
                    ┌─────────────────┐
                    │   标签库 tags    │
                    │ （全局词表 M:N）  │
                    └────────┬────────┘
         ┌───────────────────┼───────────────────┐
         │                   │                   │
         ▼                   ▼                   ▼
┌────────────────┐ ┌────────────────┐ ┌────────────────────────┐
│ tag_categories │ │ role_categories│ │ categories（菜单/产品线）│
│    （场景）     │ │    （角色）     │ │                        │
└───────┬────────┘ └───────┬────────┘ └───────────┬────────────┘
        │ via               │ via                  │ via（已落地）
        │ tag_category_id   │ role_category_tags   │ category_tags
        │ （已有）          │ （已有）              │ 已落地（见 §七）
        ▼                   ▼                      ▼
      tool_tags ───────── tool_tags ─────────── tool_tags
        （同一套工具标签边）
```

- **工具主归属（菜单维度）**：`tools.category_id` → `categories`（用于 `/category/...`、面包屑、提交表单主分类），与标签 **正交**。
- **场景维度**：`tag_categories` + `tags.tag_category_id`（与 [`docs/label.md`](./label.md) 一致）。
- **角色维度**：`role_categories` + `role_category_tags`。
- **菜单维度（对齐后）**：`categories` + **`category_tags`**（已建表），表达「该菜单分类在运营上关联哪些标签」；**不改变**「工具挂在哪个菜单分类」仍以 `category_id` 为准，除非你我在后续步骤明确升级列表算法。

---

## 三、管理后台 — 目标功能清单（验收口径）

**本轮验收**：下列 1–6 项已在代码中实现；数据库需已执行 **§七** 迁移。

完成本轮后应满足：

1. **固定标签库，可持续维护**  
   - 已有：`/admin/tags` 清洗、合并、改名、curated、归属场景等延续使用。
2. **标签库管理**：支持 **新增标签**、**禁用标签**（禁用后前台不再展示该标签页/Chip/联结筛选中的该标签，具体过滤规则在实现阶段写死并在此文档更新）。
3. **场景分类管理**（与现状对齐并文档化）：新建/禁用、向分类迁入标签、从分类移除（现有能力上以文档与字段语义收口）。
4. **角色分类管理**：新建/禁用、联结标签增删（现有能力上以文档收口）。
5. **菜单分类管理**（新增，**交互对齐**「场景分类管理」）：新建或沿用现有 `categories`、**禁用菜单分类**、**category_tags** 联结标签的添加与移除。
6. **清洗能力**：现有合并/改名/删除/一键提取等 **继续使用**，与新字段（如 `tags.is_disabled`）兼容。

---

## 四、待完成需求（本里程碑不做）

### FR-TAXO-01：详情页「工具 → 标签 → 场景 / 角色」

- 标签 Chip 可点到 `/tag/[slug]`；必要时展示父场景 `/tag-category/...`；角色侧可提供「含该标签的角色入口」或至少在文案上引导。
- 验收：从任一工具详情至少能通过点击标签到达标签页；可选再跳到场景页。

### FR-TAXO-02（预留）：用户收藏场景分类 / 角色分类

- 数据表：`user_favorite_tag_categories`、`user_favorite_role_categories`（名称待定）、与用户 id 关联。
- API / 页面入口、与账号中心联动。
- **依赖**：登录态与收藏 UX 统一方案。

---

## 五、实施计划（逐项勾选）

### Phase 0：文档冻结（无代码）

- [x] 评审本文档第二节「四条轴线」是否与产品文案一致；如需 `/category` 列表合并标签命中规则，在 Phase 3 前单独拍板。

### Phase 1：数据库 — 标签禁用 + 菜单分类禁用 + 菜单↔标签联结

- [x] 迁移：`tags.is_disabled boolean NOT NULL DEFAULT false` + 注释（禁用后前台过滤）。
- [x] 迁移：`categories.is_disabled boolean NOT NULL DEFAULT false`（默认 false，**线上行为与加列前一致**）。
- [x] 迁移：新建 `category_tags`（`category_id` → `categories`、`tag_id` → `tags`，`PRIMARY KEY (category_id, tag_id)`，`sort_order` 可选），索引 `tag_id`。
- [x] Neon / Supabase RLS：与 `role_category_tags` 策略对齐（只读给 anon/auth；写入仅服务端）。
- [x] `lib/neon/data.ts`：`neonAdmin*` 系列 — 设置标签禁用、分类禁用、`category_tags` link/unlink；公开列表查询统一 **过滤禁用标签**（含首页 chip、sitemap、标签页等，逐项清单在 PR 中自检）。
- [x] `lib/types.ts`：`Category`、`TagRow` / `AdminTagRow` 补 `is_disabled`（若适用）。

### Phase 2：管理后台 — 标签库「禁用」与创建口径

- [x] `/admin/tags`：禁用开关、列表筛选「已禁用」；禁用标签不可在前台路由与聚合中出现（与 Phase 1 查询一致）。
- [x] 新建标签入口（若仅在清洗页可用则文档说明）；与 `is_curated`、场景迁入逻辑无冲突。

### Phase 3：管理后台 — 「菜单分类管理」页面（对齐场景管理 UX）

- [x] 新页面：例如 `/admin/menu-categories`（名称可与侧栏中文统一）。
- [x] 组件：复用或抽取与 `admin-scene-category-manager` 相同的区块模式（禁用 Switch、Popover 迁入标签、行内移除）。
- [x] Server Actions：`app/admin/menu-categories/actions.ts`（命名待定）调用 neon。
- [x] 侧栏：`compact-app-sidebar.tsx` 增加入口，与「场景分类管理」同级或分组合理。

### Phase 4：行为一致性自检（前台）

- [x] **默认**：`/category/[slug]` 仍仅按 `category_id` 子树（与现状一致）。
- [x] **禁用菜单分类**：导航树、`generateStaticParams`、分类页返回 notFound 或过滤策略与 `tag_categories.is_disabled` 行为对齐（文档写明）。
- [x] **回归**：提交表单分类下拉、热门 hot、`neonListToolsForCategoryIds` 等路径不测回归。

### Phase 5：收尾

- [x] 更新 [`docs/done.md`](./done.md) 当日条目：本里程碑的需求背景、迁移文件名、实现要点。
- [x] 若 `docs/label.md` 与本文冲突，以 **本文第二节为准** 修订 `label.md` 左侧菜单描述一节（简短指向即可）。

---

## 六、进度总览

| 序号 | 内容 | 状态 |
|------|------|------|
| P0 | 文档评审 | **已完成** |
| P1 | 迁移 + Neon 数据层 | **已完成** |
| P2 | 标签库禁用后台 | **已完成** |
| P3 | 菜单分类管理后台 | **已完成** |
| P4 | 前台一致性自检 | **已完成** |
| P5 | done.md / label.md 收口 | **已完成** |
| — | FR-TAXO-01 详情标签链接 | **未做（刻意延后）** |
| — | FR-TAXO-02 收藏场景/角色 | **未做（预留）** |

---

## 七、迁移文件（已落地）

本次里程碑使用的迁移文件：

- [`supabase/migrations/20260507140000_tags_categories_disabled_category_tags.sql`](../supabase/migrations/20260507140000_tags_categories_disabled_category_tags.sql)：`tags.is_disabled`、`categories.is_disabled`、`category_tags` 表及 RLS。

---

## 八、标签清理（「待清理」）、Curated 与场景归属

### 8.1 「待清理」与 Tab「未分类」

- **标签清理页 · 视图「待清理」**：筛选条件是 **`tags.is_curated = false`**（非官方词表），**不是**「没有场景分类」。
- **Tab「未分类」**：在同一视图下，按 **`tags.tag_category_id IS NULL`** 分组展示（尚未归入任一 **`tag_categories`** 场景）。
- 二者可重叠：一条标签可以同时满足「待清理」且「未分类」；也可已是「待清理」但已选了场景分类。

### 8.2 「场景分类管理」页顶统计卡片

- **待清理标签**：**`is_curated = false`** 的标签条数。
- **待清理工具关联**：上述标签各自的 **`tool_tags` 条数（tool_count）之和**，不是「未分类」独有。
- **仅在下拉里改场景分类**不会把标签移出「待清理」；统计不变属预期，除非再 **标 Curated** 或 **合并进别的标签**。

### 8.3 运营处理「待清理」的常见路径

1. **选场景分类 → 标 Curated**（入口：标签清理表头下拉 + 「标 Curated」）。
2. **合并到已有标签**：工具关联迁到目标，`tool_tags` 随之迁移；源标签删除。
3. 顺序上可先归类再在同 Tab 或其它 Tab 操作；**逻辑上等价于**上述两步的组合。

### 8.4 硬性规则（与实现对齐）

- **`is_curated = true` ⇒ `tag_category_id` 必须非空**（必须归属某个场景 **`tag_categories`**）。不允许「场景未分类的官方词条」。
- **场景分类管理里「移出分类」**（即将 `tag_category_id` 置空）：若该标签仍是 Curated，服务端拒绝；须先在标签清理 **取消 Curated** 再移出。
- **从角色/菜单入口「新建标签」**：不写 `tag_category_id`；**不能**勾 curated 创建——须在标签清理选好场景后再标 Curated（或由场景入口新建）。

口径与计数仍以 **[`docs/tool-tag-taxonomy-counting.md`](./tool-tag-taxonomy-counting.md)** 为准；本节仅约束 **Curated ↔ 场景归属** 的管理语义。

---

*文档版本：2026-05-07 里程碑（Phase 0–5）已完成勾选；§四两项仍为后续迭代范围。**§八（2026-05-09）** 增补标签清理 / Curated / 场景归属约束。*
