/**
 * Node runtime 下静默 `TimeoutNegativeWarning`。
 *
 ...
 * 其它 warning 维持默认行为。
 *
 * 注意：本文件**只**会被 `instrumentation.ts` 在 `NEXT_RUNTIME === 'nodejs'`
 * 分支下动态 import；不要从 Edge / 客户端代码引用它。
 */
...
function isTimeoutNegativeWarning(
  ...
  if (typeof warning === 'string') {
    const optsName =
      typeof nameOrOptions === 'string' ? nameOrOptions : nameOrOptions?.type
